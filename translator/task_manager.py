...
def requeue_stalled_and_failed(workspace_paths: dict):
    ...
    if stalled_tasks:
        print(f"[TaskManager] Обнаружено {len(stalled_tasks)} 'зависших' задач. Возвращаем в очередь.")
    ...
    if failed_tasks:
        print(f"[TaskManager] Обнаружено {len(failed_tasks)} 'упавших' задач. Возвращаем в очередь.")
...
def requeue_completed_tasks(workspace_paths: dict):
    ...
    print(f"[TaskManager] Перемещено {len(tasks_in_done)} исходных чанков из 'done' в 'pending'.")


def cleanup_workspace(workspace_paths: dict):
    """Полностью удаляет рабочую директорию главы."""
    base_dir = workspace_paths["base"]
    print(f"Очистка рабочей директории: {base_dir}")
