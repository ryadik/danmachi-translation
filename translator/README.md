...
### ВАЖНО
**Мне абсолютно не хотелось заморачиваться и писать воркеры на ноде, так что я _полностью навайбил_ (через Gemini 2.5 pro) скрипт на змее, поэтому какое качество кода, я беcпонятия. Заточен только под перевод данного тайтла.**
...
#### Запуск с последующей очисткой (`--cleanup`)
Добавляет флаг `--cleanup`, который автоматически удалит рабочую директорию главы после **успешного** завершения всего процесса.

```bash
python3 translator/main.py --cleanup text/chapters/prologue/jp.txt
```
...
- [x] отладка и доработка взаимодейтсвия с глоссарием
  ...
  - [x] чо есть термин? ясно определить, что не надо выводить как термин.
- [ ] сделать удаление `workspace` поумолчанию и убрать флаг `--cleanup`
- [ ] добавить флаг `--debug` для того, чтобы `workspace` не удалялась после завершения
